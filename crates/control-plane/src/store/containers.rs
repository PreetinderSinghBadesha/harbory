...
impl Store {
    ...
    pub async fn upsert_desired_container(
        ...
        sqlx::query(
            "INSERT INTO desired_containers (agent_id, name, image, env, port_mappings, command, desired_status, updated_at)
             VALUES ($1, $2, $3, $4, $5, $6, $7, now())
             ON CONFLICT (agent_id, name) DO UPDATE
             SET image = $3, env = $4, port_mappings = $5, command = $6, desired_status = $7, updated_at = now()",
        )
        .bind(agent_id)
        .bind(&container.name)
