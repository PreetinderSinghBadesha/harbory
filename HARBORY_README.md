...
- **New (Phase 5):** the real login flow (signup, OAuth, a genuine Supabase-issued JWT reaching the real backend) has not been exercised end-to-end — creating a Supabase project needed an account only the user could create. Only the JWT verification logic and the full HTTP auth gate (both with synthetic tokens) have been tested. Do this once real Supabase credentials are in hand — see `docs/dashboard.md`'s setup checklist.
