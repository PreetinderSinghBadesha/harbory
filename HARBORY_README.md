...
- **New (Phase 2):** a revoked agent's stream connect fails (`verify_agent_credential` returns `Revoked`), but the agent's reconnect loop can't distinguish that from a transient failure — it will back off and retry forever rather than surfacing "you need to re-pair." No UX for this exists yet since revocation itself isn't exposed until Phase 5; worth a deliberate decision then about what a revoked agent process should do (exit? keep retrying quietly? both, configurably?).
